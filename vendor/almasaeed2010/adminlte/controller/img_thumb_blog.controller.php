...
if(!isset($_POST['id_post']) || empty($_POST['id_post'])){
    ...
    $img = addslashes(md5($_FILES['imagemThumb']['name'] . rand(0, 999)));
	...
	switch($_FILES['imagemThumb']['type']):
		case 'image/jpeg';
		case 'image/pjpeg';
			$imagem_temporaria = imagecreatefromjpeg($_FILES['imagemThumb']['tmp_name']);
            ...
            $resultInsert = $blog->insertImgThumb($id, $img . ".jpg");
            if($resultInsert){
                header("Location: ../blog.php?imgThumb=true&id={$id}");
            }
		...
		case 'image/png':
		case 'image/x-png';
			$imagem_temporaria = imagecreatefrompng($_FILES['imagemThumb']['tmp_name']);
            ...
            $resultInsert = $blog->insertImgThumb($id, $img . ".png");
            if($resultInsert){
                header("Location: ../blog.php?imgThumb=true&id={$id}");
            }
		break;
