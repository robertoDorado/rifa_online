...
if(!isset($_POST['id_post']) && empty($_POST['id_post'])){
    ...
    $img = addslashes(md5($_FILES['imagemPrincipal2']['name']) . rand(0, 999));
    $id = addslashes($_POST['id_post']);

    $altura = "481";
	$largura = "1010";
	
	switch($_FILES['imagemPrincipal2']['type']):
		case 'image/jpeg';
		case 'image/pjpeg';
			$imagem_temporaria = imagecreatefromjpeg($_FILES['imagemPrincipal2']['tmp_name']);
            ...
            $resultInsert = $blog->insertImagemPrincipal2($id, $img . ".jpg");
            if($resultInsert){
                header("Location: ../blog.php?imgPrincipal2=true&id={$id}");
            }
		...
		case 'image/png':
		case 'image/x-png';
			$imagem_temporaria = imagecreatefrompng($_FILES['imagemPrincipal2']['tmp_name']);
            ...
            $resultInsert = $blog->insertImagemPrincipal2($id, $img . ".png");
            if($resultInsert){
                header("Location: ../blog.php?imgPrincipal2=true&id={$id}");
            }
		break;
